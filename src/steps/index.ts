...
export const STEPS: StepConfig[] = [
...

  {
    ...
    description: h('div', null,
      ...
      h('p', null, 'An ', h('code', null, 'IfNode'), ' has three parts:'),
      h('ul', null,
        h('li', null, h('code', null, 'node.predicate'), ' — the condition'),
        h('li', null, h('code', null, 'node.statements'), ' — the then-branch body'),
        h('li', null, h('code', null, 'node.consequent'), ' — an ElseNode; access its ', h('code', null, '.statements'), ' for the else-branch body'),
      ),
    ),
...

  {
    ...
    description: h('div', null,
      ...
      h('p', null,
        'Takes two arguments: ', h('code', null, 'mid'), ' (method ID — the method name as a symbol, e.g. ', h('code', null, ':fib'), ') and ', h('code', null, 'iseq'), ' (the compiled method body).',
      ),
      h('ul', null,
        h('li', null, 'Register the method\'s iseq on the current class with the given name'),
      ),
      h('h3', null, 'Instruction: OptSendWithoutBlock'),
      h('p', null,
        'Takes one argument: ', h('code', null, 'cd'), ' (call data — a small object holding the method name and argument count).',
      ),
      h('ul', null,
        h('li', null, 'Dispatch a method call using the call data'),
      ...
      h('ul', null,
        h('li', null, 'Compile the method body into a separate iseq (check the ', h('strong', null, 'Iseq API Reference'), ' for how to create a method iseq)'),
        h('li', null, 'Emit ', h('code', null, 'Definemethod'), ' to register it on the VM'),
        h('li', null, 'In Ruby, ', h('code', null, 'def foo'), ' returns the method name as a symbol — push it as the expression\'s return value'),
      ),
      ...
      h('p', null,
        h('code', null, 'CallData'), ' is a small data object that tells the VM which method to call. Create it like this:',
      ),
      h('pre', null, h('code', null,
        'YRuby::CallData.new(mid: :fib, argc: 1)'
      )),
      h('p', null,
        h('code', null, 'mid'), ' is the method name (use ', h('code', null, 'node.name'), '), ',
        h('code', null, 'argc'), ' is the argument count (use ', h('code', null, 'node.arguments.arguments.length'), ').',
      ),
      h('ul', null,
